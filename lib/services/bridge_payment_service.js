    ...
    try {
      _this._postBridgePaymentToDestination(addresses.destination.domain, bridgePayment)
        .then(function(destinationBridgePayment) {
          var ripplePaymentRequest = _this._buildRipplePaymentRequest(bridgePayment.wallet_payment, bridgePayment.source.address);
...
BridgePaymentService.prototype._buildRipplePaymentRequest = function (wallet_payment, address) {
  ...
  return {
    ...
    destination_tag: dtIndex > 0 ? wallet_payment.destination.substr(dtIndex+1) : ''
  };
};
