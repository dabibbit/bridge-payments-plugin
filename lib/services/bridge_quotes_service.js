  ...
  return new Promise(function(resolve, reject) {
    _this._validateRequest(requestParams)
      .then(function(validatedRequest) {
        Promise.props({
          source: _this._generateSource(validatedRequest.sender),
          destination: _this._generateDestination(validatedRequest.receiver)
...
BridgeQuotesService.prototype._generateSource = function _generateSource(address) {
  var _this = this;
  return new Promise(function(resolve, reject) {
...
BridgeQuotesService.prototype._generateDestination = function _generateDestination(address) {
  var _this = this;
  return new Promise(function(resolve, reject) {
    var addressArray = address.split(':');
    if (addressArray.length !== 2 || !addressArray[0] || (addressArray[0] !== 'acct' && addressArray[0] !== 'ripple') || !addressArray[1]) {
      reject(new Error('Destination formatting error'));
    }
      ...
      rippleName.lookup(destinationAddress)
        .then(function(user){
          if (!user.exists) {
            ...
            reject(new Error('Destination address not a valid ripple address'));
          } else {
            resolve({
              uri: address
            });
